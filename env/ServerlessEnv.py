...
class ServerlessEnv(gym.Env):
    ...
    def summary_graphs(self, write_graph):
        ...
        # Episodic_failure_rate_summary += self.worker.Episodic_failure_rate
    ...
    def calculate_reward(self, step_c, ac1, ac2):
        ...
        if ac1 == 0:
            ...
            # if self.worker.ec2_vm_up_time_dict[self.worker.ec2_vms[ac2]]['status'] != "ON" and self.worker.ec2_vms[ac2] not in self.worker.pending_vms:
            #     self.worker.req_cost = (self.worker.ec2_vms[ac2].price/3600) * (self.worker.fn_features[str(self.current_request.type) + "_req_MIPS"] / self.worker.fn_features[str(self.current_request.type) + "_cpu_req"])
            # else:
            #     self.worker.req_cost = 0
